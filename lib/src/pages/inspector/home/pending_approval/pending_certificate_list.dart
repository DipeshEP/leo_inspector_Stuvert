...
import 'package:leo_inspector/Model/certificate_details_model.dart';
...
import '../inspection/fill_checklist.dart';
import '../../../../../Model/inspector_model2.dart';

class PendingCertificatePage extends StatefulWidget {

  final String? inspectorId;
  final Inspector? authorisedInspector;
  final String? clientId;

  const PendingCertificatePage({
    Key? key,
  ...
  @override
  State<PendingCertificatePage> createState() => _PendingCertificatePageState();
}

class _PendingCertificatePageState extends State<PendingCertificatePage> {
  ...
  getCertificateList() async{
    try{
      ...
      Response? response = await inspectorRepository.getCertificateList(null, 'Pending', widget.inspectorId!,widget.clientId);
        ...
        child: ListView.builder(
            ...
            itemBuilder: (BuildContext context, int index) {
              return Padding(
                ...
                child: InkWell(
                  onTap: (){
                    routeToEdit(_certificateList[index]);
                  },
                  child: Container(
                    decoration: BoxDecoration(
                        color: Colors.white,
  ...
  routeToEdit(Certificate certificate) async{
    try{
      String rowData = '';
      rowData = certificate.certificateRowData!;
      CertificateDetailsModel? certificateDetailsModel = certificateDetailsModelFromJson(rowData);
      if(certificateDetailsModel != null){

        print('certificateDetailsModel.surveyInspector');
        print(certificateDetailsModel.surveyInspector!.inspectorId);
        print(certificateDetailsModel.surveyInspector!.name);

        Navigator.push(
            context,
            MaterialPageRoute(
                builder: (context) => FillCheckListPage(
                  questionOption: certificateDetailsModel.questionOption,
                  companyId: certificate.cmpId,
                  clientId: certificate.clientId,
                  isToBeAuthorised: true,
                  authorisedInspector: widget.authorisedInspector,
                  surveyInspector: certificateDetailsModel.surveyInspector,
                )));
      }
    }
    catch(e){
